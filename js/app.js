...
// Версия 3.4.2 - Отключен svgbob (WASM несовместим с CDN)
...
(async function() {
    ...
    // ASCII art support
    // Note: svgbob-wasm cannot be loaded via CDN in browser-only environment
    // WASM binary files are not compatible with ESM module loaders (jsdelivr, skypack, unpkg, esm.sh)
    // Using high-quality fallback: formatted code blocks with Unicode box-drawing character support
    let svgbobRender = null;
    console.log('[INFO] ASCII diagrams will be rendered as formatted code blocks with Unicode support');
